...
function generateGridData() {
    ...
    for (let v = 0; v < numValleys; v++) {
        ...
        const valleySize = Math.floor(Math.random() * 3) + 2; // Radio de 2-4 celdas
...
function handleCellClick(event) {
    ...
    showMessage(`Punto de inicio seleccionado en (${currentCol + 1}, ${15 - currentRow}). Presione "Comenzar" para iniciar.`, 'info');
}
...
// Find the best neighbor ("two times cross" - incluye vecinos lejanos)
function findBestNeighbor() {
    const neighbors = [
        // Cruz interna (adyacentes directos)
        { r: currentRow - 1, c: currentCol, order: 1 },     // Arriba
        { r: currentRow, c: currentCol - 1, order: 2 },     // Izquierda
        { r: currentRow + 1, c: currentCol, order: 3 },     // Abajo
        ...
        // Cruz externa (vecinos lejanos a distancia 2)
        { r: currentRow - 2, c: currentCol, order: 9 },     // Arriba lejano
        { r: currentRow, c: currentCol - 2, order: 10 },    // Izquierda lejano
        { r: currentRow + 2, c: currentCol, order: 11 },    // Abajo lejano
        { r: currentRow, c: currentCol + 2, order: 12 }     // Derecha lejano
    ];
    ...
    setTimeout(() => {
        ...
        if (bestNeighbor) {
            ...
            showMessage(`Moviéndose a (${currentCol + 1}, ${15 - currentRow}) con valor ${gridData[currentRow][currentCol]}`);
        } else {
            // No better neighbor found, animation stops
            clearInterval(intervalId);
            intervalId = null;
            showMessage(`Óptimo local encontrado en (${currentCol + 1}, ${15 - currentRow}) con valor ${gridData[currentRow][currentCol]}`, 'success');
